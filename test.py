...
# Testing ASPP with random input
if __name__ == "__main__":
    batch_size = 1  # Number of images in batch
    in_channels = 2048  # Simulating output from ResNet
    height, width = 32, 32  # Feature map size

    input_tensor = torch.randn(
        batch_size, in_channels, height, width)  # Create random input

    aspp = ASPP(in_channels=in_channels)  # Initialize ASPP
    output = aspp(input_tensor)  # Forward pass

    print("Input shape:", input_tensor.shape)  # (1, 2048, 32, 32)
    print("Output shape:", output.shape)  # Expected: (1, 256, 32, 32)
